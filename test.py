...
async def on_message(message):
	if message.type == nextcord.MessageType.premium_guild_subscription:
		channel = bot.get_channel(1072289395593461790)
...
async def on_member_remove(member):
	...
	embed.set_image(
	 url=
	 "https://cdn.nextcordapp.com/attachments/1072289395593461790/1044440675451674724/giphy.gif"
	)
	embed.timestamp = datetime.datetime.today()
	await channel.send(embed=embed)
...
async def on_presence_update(before, after):
    ...
    if before.activities == after.activities:
        return
    role = nextcord.utils.get(after.guild.roles, name="Verfify")
    required_activity = "discord.gg/WM2QnFZQHN"
    ...
    if userHasActivity:
        ...
        channel = bot.get_channel(1072289395593461790)
        embed = nextcord.Embed(
            title="Tizi-Development",
            description=
            f"{role.name} wurde {after.mention} hinzugefügt, weil Tizi-Developer im Status hinzugefügt wurde!"
        )
        embed.set_thumbnail(url=after.avatar)
        yourServer = after.guild
        if 'Verfify' not in str(before.roles):
            if 'Verfify' in str(after.roles):
                channel = bot.get_channel(1072289395593461790)
                await channel.send(embed=embed)
        ...
        yourServer = after.guild
        if 'Verfify' in str(before.roles):
            if not 'Verfify' in str(after.roles):
                channel = bot.get_channel(1072289395593461790)
                await channel.send(embed=nextcord.Embed(
                    title="Tizi-Development",
                    description=
                    f"{role.name} wurde von {after.mention} entfernt, weil Tizi-Developer aus dem Status entfernt wurde!"
                ))
                await channel.purge(limit=0)
...
async def serverinfo(interaction):
    ...
    embed.add_field(name='Server erstellt am', value=guild.created_at.strftime('%d.%m.%Y %H:%M:%S'), inline=False)
    ...
    embed.add_field(name='Bot-Latenz', value=f'{round(interaction.bot.latency * 1000)}ms')
    await interaction.response.send_message(embed=embed)
